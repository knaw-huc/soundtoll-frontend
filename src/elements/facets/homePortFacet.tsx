...
function HomePortFacet(props: {parentCallback: ISendCandidate}) {

    let [more, setMore] = useState(true);
    const [filter, setFilter] = useState("");
    let [data, setData] = useState<facetList>({"buckets": []});
    let url: string = SONT_SERVICE + "elastic/initial_facet/schipper_plaatsnaam/short";
    const [help, setHelp] = useState(false);
    const facets: facetList = data;

    async function fetchData() {
        if (more) {
            url = SONT_SERVICE + "elastic/facet/schipper_plaatsnaam/short/" + filter;
        } else {
            url = SONT_SERVICE + "elastic/facet/schipper_plaatsnaam/long/" + filter;
        }
    ...
    function changeListLength() {
        if (more) {
            if (filter === "") {
                url= SONT_SERVICE + "elastic/initial_facet/schipper_plaatsnaam/short";
            } else {
                url= SONT_SERVICE + "elastic/facet/schipper_plaatsnaam/short/" + filter;
            }
            setMore(false);
            ...
            if (filter === "") {
                url= SONT_SERVICE + "elastic/initial_facet/schipper_plaatsnaam/long";
            } else {
                url= SONT_SERVICE + "elastic/facet/schipper_plaats/longnaam/" + filter;
            }
            setMore(true);
        }
    }
        ...
        <div className="hcFacet">
            ...
            <div className="hcFacetItems">
                {facets.buckets.map((item) => {
                    return (
                        <div className="hcFacetItem" onClick={() => props.parentCallback({facet: "Home port", field: "schipper_plaatsnaam", candidate: item.key})}>
                            {item.key}
                        </div>
                    )
                })}
